...
| A18 | **U3 bulge prominence:** mostly ordered (the top end, obvious → dominant on 38 test galaxies, is underpowered), and tracks measured B/T both ways (agreement 0.72; Experiment D reads BOTH) | U3 → V2 → W1 · same | SUPPORTED WITH CAVEAT (ordering); "both ways" PENDING re-run | Simard 2011 B/T is a model decomposition with its own degeneracies. **"Both ways" (V2 Experiment D) is unverified:** Y3 showed that design reads BOTH on two noisy copies of one quantity | Re-run Experiment D with Y3's cross-decoder controls and matched shared null | — |
