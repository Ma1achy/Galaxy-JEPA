...
| A18 | **U3 bulge prominence:** mostly ordered (the top end, obvious → dominant on 38 test galaxies, is underpowered), and tracks measured B/T both ways (agreement 0.72; Experiment D reads BOTH) | U3 → V2 → W1 · same | SUPPORTED WITH CAVEAT | Simard 2011 B/T is a model decomposition with its own degeneracies | — |
...
| A19 | **U3 winding:** ordered along an axis fitted on the extremes | U3 → W1 · same | SUPPORTED (ordering) · **PENDING (physical grounding) on Brief Y** | No public machine-measured pitch-angle catalogue (W3); Yu & Ho 2020 released a 10-row excerpt | **Brief Y:** PyArcFiRe pitch angles on our stamps (deprojected, mirror-validated); the drafted data requests |
| A20 | **U3 arm count:** ordered | U3 → V1 → W1 · same | SUPPORTED WITH CAVEAT — **not established as morphology** | Visibility falls monotonically from "1 arm" to "4+"; can't-tell galaxies are *more* visible (AUC 0.40) | Arm count from PyArcFiRe (Brief Y) at matched visibility |
