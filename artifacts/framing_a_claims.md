...
| A19 | **U3 winding:** ordered along an axis fitted on the extremes. Against machine-measured pitch (Brief Y): the votes' ordering tracks SpArcFiRe pitch **beyond visibility** (τ 0.25 → 0.22 Hayes; 0.23 → 0.23 Hart), and w_avg agrees with it (Hayes ρ −0.35 on the all-arcs estimator; **WEAK AGREEMENT under the primary |DCO| estimator, Y4: ρ −0.31, partial −0.28**; Hart −0.32). The encoder's winding is vote-like: VOTES BEYOND MEASUREMENT (A_v 0.27; A_m 0.097), pitch decode margin over untrained only +0.02 to +0.06; its axis meets pitch mostly through visibility (retention 0.42) | U3 → W1 → Y · `u_findings.md`, `y_findings.md` §Y2–Y4. Hayes table: method Davis & Hayes 2014 (ApJ 790, 87); GZ1-scale run Hayes, Davis & Silva 2017 (MNRAS 466, 3928) and Peng et al. 2018 (MNRAS 479, 5532); P_CS from Lintott et al. 2011; file, source, access date and hashes in §Y4. **P_CS = P_EDGE + P_CW + P_ACW, so the table is selected on volunteer votes** | SUPPORTED WITH CAVEAT (ordering is physical within SpArcFiRe) | **Pitch angle is not a usable independent reference (Z1: DISAGREE).** Hart (SpArcFiRe) vs Yu & Ho (2DFFT): ρ −0.25, 95% upper bound +0.10, n 34 (the whole overlap), against the ≥ 0.5 two published methods should reach. So "physical" here means "within SpArcFiRe". Yu & Ho 2DFFT also agrees with neither the votes nor SpArcFiRe (n 550, 376); Hayes vs Hart only ρ 0.38 (0.43 on |DCO|; 0.47 with the exploratory 'agree' filter); Hayes is selected on P_CS > 0.5 and disc axis ratio > 0.5 (fainter, smaller than our spirals) | A larger 2DFFT sample; PyArcFiRe on our own stamps (parked) |
...
| A22 | The top two components (PC1 24.9%, PC2 11.9%) are **learned** and are **not morphology**: they are **band misregistration** — the sub-pixel offsets between the g, r and i stamps (AA3a: INSTRUMENTAL, R² 0.83 / 0.84; causal arm RESPONDS, a 1 px g-only shift moves PC1 by +0.67). **Harmless to the linear readout** (AA3b, exploratory: projecting them out changes existence by a median −0.0007; the pre-registered D4-average state MIXED is test-time ensembling, +0.013 → +0.021 with 2 → 8 views). Not handedness (X1) | W2, X1, AA3a, AA3b · `aa_findings.md` §AA3a–AA3b · `assets/pose_code.png`, `assets/pose_average.png` | SUPPORTED | Cause located in `sciserver_cut.py:81` (per-band `Cutout2D`, integer snapping on each band's own grid); the CasJobs `colc`/`rowc` confirmation is not run. The ensembling reading is post hoc and labelled exploratory; salience moves were not separated the same way | Registered-band retrain (needs a D-entry; hygiene, not expected to move the catalogue) |
...
| B6 | The band-misregistration code in PC1/PC2 (A22) is picked up by any objective, not only JEPA; registration removes it and augmentation does not | One encoder's PCs cannot be compared across objectives | MAE / MoCo baselines through W2 + X1 | PENDING |
