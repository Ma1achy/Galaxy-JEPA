...
- **Masking must target the galaxy.** Block-masking a mostly-black SDSS cutout wastes budget on empty sky. Bias mask sampling toward the galaxy — reuse v1's average-image bounding box. The **Paper-1 default** is a **per-galaxy box scaled by the Petrosian radius** (already pulled for the nuisance battery, so nearly free), which handles apparent-size variation a single global box cannot — loose on small/distant galaxies, clipping large/near ones; the global average-image box is kept as the β/ablation fallback. Full scheme + the β-generalisation (β=0 ⇒ standard I-JEPA) in `docs/masking.md`.
...
- [~] Masking specifics — **proposed in `docs/masking.md`**: bounding-box-biased multi-block with a single bias strength β, a strict generalisation of I-JEPA (β=0 ⇒ standard I-JEPA), so β ∈ {0, 0.5, 1.0} is a free ablation. **Paper-1 default box = per-galaxy, scaled by the Petrosian radius** (already pulled for the nuisance battery, so nearly free); the global average-image box is retained as the β/ablation fallback. Open sub-points: the size multiplier *k* and behaviour when Petrosian radius is missing; and re-tuning the EMA schedule / masking ratio per β, since biasing all targets onto the galaxy shifts the prediction-difficulty distribution (removing the easy, collapse-adjacent sky-prediction task).
