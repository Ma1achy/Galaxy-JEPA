...
The rung is **not an intrinsic property of the feature** — it's a joint property of *(feature × SSL objective × architecture/resolution × probe)*. These confounds, each with its control:
...
- **Architecture (backbone)** → backbone ablation: clean ViT → conv-stem hybrid (CCT/CvT) → E(2)-equivariant ViT. This is the *architecture* term of the decomposition made into a control, peer to the patch-size and cross-objective controls — a feature could be Rung 3 under a clean ViT but Rung 1 under a conv stem or an equivariant prior. The Paper-1 default is a **clean ViT** for an I-JEPA-specific reason: a conv stem's receptive field **bleeds target-region pixels into context tokens *before* masking**, leaking the exact masking mechanism this project is built to study (and muddying the β sky-fraction diagnostic). Clean ViT keeps the masking semantics unambiguous — principled, not inherited from v1.
...
- **Masking must target the galaxy.** Block-masking a mostly-black SDSS cutout wastes budget on empty sky. Bias mask sampling toward the galaxy — reuse v1's average-image bounding box. The **Paper-1 default** is a **per-galaxy box scaled by the Petrosian radius**, which handles apparent-size variation a single global box cannot — loose on small/distant galaxies, clipping large/near ones; the global average-image box is kept as the β/ablation and missing-radius fallback. **Note the corpus:** masking runs during *pretraining*, which (per D6) is the large *unlabelled* SDSS sample — **not** the GZ2 probing set the nuisance battery covers. So the radius must be a **distinct pull on the pretraining corpus** (petroRad from SDSS PhotoObjAll — present for *all* photometrically-detected galaxies, not just spectroscopic ones — plus the cutout's arcsec/pixel to convert to pixels). Cheap, but not free and not covered by the nuisance pull. Reaching ≫250k unlabelled galaxies likely goes fainter than the GZ2 spectroscopic limit (r < 17.77), where petroRad gets noisier → per-galaxy boxes degrade on the faint end (the global-box fallback and the *k* slack absorb this), with a mild pretraining-vs-probing distribution shift (fainter, smaller apparent size). Full scheme + the β-generalisation (β=0 ⇒ standard I-JEPA) in `docs/masking.md`.
- **Preprocessing parity — format + stretch + normalisation, byte-identical everywhere.** Pretraining and probing decouple (D6), but the *preprocessing* must not: format, dynamic-range stretch, and normalisation are identical across the pretraining corpus, the probing corpus, and every baseline — pull both corpora the same way, never mix pipelines. **Pull FITS, not pre-stretched cutouts:** 8-bit display-stretched cutouts irreversibly compress the low-surface-brightness range where the confused features live (winding, arm count, tidal structure), so probing on them would confound a genuine **Rung-4** result (*absent from the pixels*) with display-stretch loss (*destroyed by the quantisation*). Apply an identical **asinh** stretch (softening `Q` + per-channel flux scale) chosen once on the pretraining corpus and **stamped as config** (it enters the run-stamp, never a notebook constant); fit normalisation **after** the stretch, once, then freeze and share. A cheap **stretch-sanity check** runs before pretraining — faint arms survive stretch+normalise while the sky-noise floor stays controlled — pairing with the collapse monitor (encoder modelling noise ⇒ stretch too aggressive). Engineering detail in `docs/spec/data.md`.
- **No-rebin is a *proven* Rung-4 protection, not a preference (resampled cutouts rejected).** Native frames are slow to pull (hard SDSS per-IP HTTP throttle ~1 MB/s; ~250k native = ~11.5 days), so a resampled FITS-cutout service (`hips2fits`, CDS `P/SDSS9`) was tested as a ~40× faster corpus path. It is **calibrated** (flux ratio to native 1.005, not display-scaled) and the **bright signal survives** (feature-flux ratio 1.00) — but that is the trap: it **attenuates high-frequency power to 0.115 of native (~89% lost)** and **correlates the pixel noise (lag-1 autocorr 0.017→0.443)**, i.e. it injects a learnable fake-structure noise field and smooths exactly the faint regime Rung-4 lives in. A resampled corpus makes *"absent from the pixels"* uninterpretable. So `native 0.396″/px, no rebin` stays — empirically justified (`artifacts/fidelity_test.py`), the same fidelity-over-convenience call as FITS-vs-cutouts.
- **Corpus pull path = SciServer Compute (confirmed, native fidelity).** Direct SDSS HTTP is infeasible at corpus scale (hard per-IP throttle ~1 MB/s, parallelism-proof; 10 MB/galaxy → ~11.5 days for 250k). The fix is to run the cutout server-side where the SDSS SAS volume is mounted, so only ~50 KB stamps cross the link, never the 10 MB frames. Verified on SciServer: a server-side `Cutout2D` from the mounted frame is **byte-identical** to the HTTP-pulled native stamp (max|Δ|=0.0 — so the frozen stretch transfers), passing the same fidelity test hips2fits failed (sky lag-1 autocorr 0.026 white / high-k frac 0.683 full). Throughput on a 32-core container: serial 0.22 gal/s, **parallel 3.79 gal/s (17.6× — no throttle on the mounted volume)** → **250k ≈ 18 h, 1M ≈ 73 h** (one-time, chunked across jobs). The data-source problem is solved at full fidelity — no Globus, no rescoping, no resampling compromise.
...
- **SSL baselines = a control, not box-ticking.** Run the *same probe ladder* on JEPA vs **MAE** (latent prediction vs pixel reconstruction) vs **contrastive (MoCo / SimCLR / BYOL)**. Cross-objective comparison is what separates *intrinsic to the images* from *artefact of how JEPA organises the manifold* — e.g. a feature that's Rung 3 in JEPA but Rung 1 in contrastive is an objective effect, not an image fact.
  - **Every cross-objective baseline must be trained on the *same SDSS pretraining corpus* as the JEPA.** The Rung-3 control holds the **dataset** fixed and varies the **objective**, so a rung difference attributes to JEPA-vs-MAE rather than to the images. The publicly released Wu & Walmsley MAE is **Euclid-trained**, so comparing an SDSS JEPA against it would vary objective *and* instrument at once — confounding exactly what the control isolates. So the released Euclid MAE is a **reference / way to validate our reimplementation**, not the controlled baseline.
  - **Fix the probe layer — penultimate, matched across all three objectives; layer is not optimised per result.** MAE's *final* block specialises for its decoder and probes markedly worse than its intermediate layers (a well-replicated result), whereas JEPA and contrastive probe fine at the last layer — so a last-layer-only ladder would confound a genuine rung difference with a layer-choice artefact and make MAE look artificially weak. Read a **fixed penultimate block** for all three for the headline; report the full AUC-vs-depth profile as a **supplementary** analysis. A best-layer *sweep* is rejected: choosing the layer that maximises probe AUC makes depth a researcher degree of freedom that contaminates selectivity / nuisance-clearance. (Engineering detail in `docs/spec/encoder.md`.)
  - **MAE specifically:** reproduce their public recipe on the SDSS corpus — it's public (a ViT, ~30M params, 3-layer decoder, **8×8 patches** — note 8×8 is the same patch size as the D11 Rung-4 control).
  - **Contrastive specifically — MoCo (decided), not BYOL.** BYOL is negative-free + EMA-target, *too architecturally close to JEPA* for a clean Rung-3 contrast; MoCo's explicit negatives are a genuinely different objective, and it is the established galaxy-SSL baseline (Hayat et al., on SDSS). SDSS-trained like the rest.
...
- **Stamp size clips the largest galaxies — a characterised corpus limitation.** Stamps are cut at **256 px** native (= the encoder input dim; no-rebin forbids resizing, so the two are the same number). Measured on a 5 000-galaxy probe sample, this leaves **~1.4% of galaxies** (`petroRad_r > 20.3″`) with their faint-outskirt annulus `[R_petro, 2.5·R_petro]` off-frame — and that 1.4% is **not random: it is enriched in the large, extended, morphology-rich galaxies the nameability/uncertainty probes most want**. So the Paper-1 corpus *systematically degrades* the giant-galaxy tail: those objects fall back to the global masking box and are excluded from the `T2` faint-retention metric. The trade was taken deliberately — 256 px is the balance of clip rate (down 5× from a 64 px stamp's 6.96%) against **per-step training compute** (320 px = 400 tokens ≈ 1.5× the ViT attention cost on *every* from-scratch pretraining step, for only 0.4–0.7 pp fewer clips), and the residual tail (giants, `R>20″`) is unrecoverable at native resolution regardless. State this limitation plainly in the paper; don't claim coverage of the most extended morphologies. Engineering detail + percentile table in `docs/spec/data.md` §3.
...
- [x] **Nuisance metadata — tractable.** GZ2 → SDSS join via **CasJobs / SkyServer** (`zoo2MainSpecz` ⋈ `PhotoObjAll` on `dr7objid = objID`, join key verified by a 10-row ra/dec agreement check before trusting it); z, apparent magnitude, Petrosian radius complete for the GZ2 spectroscopic main sample (it's built on SDSS DR7 main-sample galaxies). **The "SNR" nuisance is the *photometric, image-domain* r-band SNR** (`≈ 1.0857 / modelMagErr_r`) — what the encoder actually sees — **not** the spectroscopic `snMedian`, which measures the fibre spectrum (wrong domain for an image-quality probe). PSF width = `psfWidth_r`. *(Confirm exact completeness fractions when pulling.)*
- [ ] Confidence — probe target, validation axis, or both? (Lean both; different questions.)
- [~] **Backbone — an ablation axis, not a one-time pick.** Paper-1 *default* is a **clean ViT-S/16** (see the architecture confound above and the I-JEPA masking-leak reason). The backbone sweep — clean ViT → conv-stem hybrid (CCT/CvT) → E(2)-equivariant ViT — is a **rung confound control**, peer to the patch-size (Rung-4) and cross-objective (Rung-3) controls, not a perf upgrade deferred to "ViT-B later". **Depends on the pretrain-corpus decision:** from-scratch ViT is viable *conditional on* the larger unlabelled corpus; if the corpus stays thin, v1's conv-stem **CCT is the data-efficient fallback**. Honest caveat: Zoobot settled on ConvNeXt/MaxViT — so make **no "ViT is best for galaxy morphology" claim** anywhere.
- [x] **Pretrain corpus — decided: decouple from the probing corpus.** Pretrain on a *large unlabelled SDSS* sample (≫250k); probe on the GZ2-labelled ~250k. Pretraining needs no labels, so there's no reason to cap it at the labelled set — and from-scratch I-JEPA on only ~250k is thin (I-JEPA used 1.3M–14M), the biggest risk to the from-scratch call: an undertrained encoder can't tell "info absent" from "encoder never learned it". Stays **single-survey** (Paper 1); also more on-thesis (the LSST "unlabelled deluge" framing). The unlabelled pull must also fetch **petroRad + arcsec/pixel scale** per galaxy (for the per-galaxy masking box — distinct from the nuisance-battery pull). Multi-survey scaling = Paper 2. (See DECISIONS.md D6.)
- [ ] From-scratch vs warm-start the encoders from an ImageNet ViT? (From-scratch is cleaner.)
- [~] Masking specifics — **proposed in `docs/masking.md`**: bounding-box-biased multi-block with a single bias strength β, a strict generalisation of I-JEPA (β=0 ⇒ standard I-JEPA), so β ∈ {0, 0.5, 1.0} is a free ablation. **Paper-1 default box = per-galaxy, scaled by the Petrosian radius**; the global average-image box is retained as the β/ablation and missing-radius fallback. Note the radius is a **distinct pull on the unlabelled *pretraining* corpus** (petroRad from PhotoObjAll + arcsec/pixel) — masking runs in pretraining, not on the GZ2 probing set the nuisance battery covers; noisier on the faint end (fallback + *k* slack absorb it). Open sub-points: the size multiplier *k* and the missing/unreliable-radius fallback; and re-tuning the EMA schedule / masking ratio per β, since biasing all targets onto the galaxy shifts the prediction-difficulty distribution (removing the easy, collapse-adjacent sky-prediction task).
...
- [~] **Split policy — leak-impossible by construction (proposed in `docs/spec/splits.md`).** The split exists to protect *a probe number you can believe*, so every way a split can lie is closed **in code**, not by discipline. Two corpora, split separately (D6): the unlabelled **pretraining** pull → `pretrain-train` + a small fixed monitor slice (collapse watch only, never probed); the GZ2 **probing** pull → `probe-train`/`val`/`test`. The two share *galaxies* (the unlabelled pull isn't GZ2-aware), and that overlap is the headline leak: a galaxy in both means the frozen encoder *memorised* a probe galaxy rather than *transferring* to it — D6 silently void, no error raised. **Guard, structural:** cross-corpus dedup on SDSS `objID` (not ra/dec floats), with a merge-blocking invariant test, so a probe galaxy in pretraining *cannot* be committed. A second, orthogonal firewall protects uncertainty geometry: the ambiguous middle (0.2 < v < 0.8) is held out of axis estimation in code (`assert_uncertainty_firewall`) — recovering a vote fraction from an axis trained on it is the tautology this whole result must avoid. Splits are seed-reproducible (SHA-256 of `objID`+seed, stamped as config — no split file to drift) and stratified on the hard/confused features + the large extended galaxies the stamp clips. **Forks flagged for sign-off:** collision resolution (recommend: shared galaxy → *probing*, dropped from the abundant pretraining pool), train/val/test ratio (recommend 70/15/15), and the pretrain-monitor split (recommend fixed, not rotating, so the collapse curve stays comparable). Guards + tests built now; the pull/ratio/stratify orchestration is deferred to the corpus pull.
