...
7. **A resume continues the same run, or it refuses.** `callbacks/checkpoint.py` carries the EMA target encoder as its own state (re-deriving it from the online encoder would restart the EMA from a copy and look clean), writes through temp → fsync → rename with the manifest as the commit point, and checks `config_hash`, `normalisation_hash`, the schedule and the schema *before* touching a weight. Resuming demands a `ResumableShuffle` sampler, because the trajectory is a function of `(seed, step)` only if the data order is; with `shuffle=True` a resumed run would train on a different sequence from the same step. The property is executable and exact, not approximate: `test_a_resumed_run_is_identical_to_an_uninterrupted_one` asserts **equality** of all 20 losses between a straight run and a stop-resume run started from a different weight init. A resume that runs but diverges is worse than no resume.
8. **The collapse kill criterion is pre-registered, not judged at hour thirty.** `CollapseFloorFreeze` is a `FrozenChoice` like the normalisation statistic and the effect floor — hashed into `config_hash`, stamped onto the artefact, enforced by `CollapseMonitor`. Leaving it unset stamps `collapse_floor_open`. A criterion chosen after seeing the curve is not a criterion.
