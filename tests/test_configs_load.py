...
def test_sigreg_is_adopted_and_changed_no_other_field():
    ...
    rather than a false "same run" (``core/config.py``). What can be pinned — and is the thing
    that matters for reading an artefact back — is that nothing *else* shifted: strip the four
    SIGReg keys and the hash is the one H5's arms were stamped with, so a D17-era
    ``config.json`` still compares field for field against a D18 one.
    """
